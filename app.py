...
PERFIS_CONFIG_DEFAULT = {
    "Conservador": {
        "lambda_poisson": 1.5,
        "perc_saldo_aposta_min": 0.05, # 5%
        "perc_saldo_aposta_max": 0.15, # 15%
        "prob_decidir_apostar": 0.6, # 60% de chance de apostar
        "cor": "#1f77b4" # Azul padrão plotly
    },
    "Moderado": {
        "lambda_poisson": 2.5,
        "perc_saldo_aposta_min": 0.10, # 10%
        "perc_saldo_aposta_max": 0.25, # 25%
        "prob_decidir_apostar": 0.8, # 80% de chance de apostar
        "cor": "#ff7f0e" # Laranja padrão plotly
    },
    "Arriscado": {
        "lambda_poisson": 3.5,
        "perc_saldo_aposta_min": 0.15, # 15%
        "perc_saldo_aposta_max": 0.40, # 40%
...
def calcular_valor_aposta(saldo_usuario, perfil_usuario_config):
    ...
    perc_aposta = random.uniform(perfil_usuario_config["perc_saldo_aposta_min"],
                                 perfil_usuario_config["perc_saldo_aposta_max"])
    valor_proposto = saldo_usuario * perc_aposta
...
st.title("🎲 Simulador de Casa de Apostas Avançado")
...
with st.sidebar:
    ...
    st.markdown("---")
    st.subheader("Configurações da Casa e Jogos")
    margem_casa_input = st.slider("Margem da Casa nas Odds (%)", min_value=0.0, max_value=20.0, value=5.0, step=0.1, format="%.1f%%") / 100.0
    num_jogos_rodada_input = st.number_input("Número de Jogos por Rodada", min_value=1, max_value=20, value=5, step=1)
    # Removido seletor - sempre 3 resultados (V, E, D)
    ...
    pode_continuar = all(
        st.session_state.perfis_config_dinamico[p]["perc_saldo_aposta_min"] < st.session_state.perfis_config_dinamico[p]["perc_saldo_aposta_max"]
        ...
        if st.button("▶️ Iniciar / Próxima Rodada", type="primary", use_container_width=True, disabled=not pode_continuar):
            ...
            if st.session_state.rodada_atual == 1:
                ...
                for i in range(num_usuarios_input):
                    perfil_usuario = random.choice(LISTA_PERFIS)
            ...
            # 1. Gerar Jogos
            for i in range(num_jogos_rodada_input):
                resultados_desc = ["Vitória", "Empate", "Derrota"]
                
                probs_reais = gerar_probabilidades_futebol()
                odds_casa = calcular_odds_casa(probs_reais, margem_casa_input)
                
                jogos_nesta_rodada.append({
                    "id_jogo": f"JOGO_{st.session_state.rodada_atual}_{i+1}",
                    "descricao": f"Jogo {i+1}",
                    "resultados_possiveis": resultados_desc,
                    "probabilidades_reais": probs_reais,
                    "odds_casa": odds_casa,
                    "resultado_final": None, "idx_resultado_final": -1
                })

            # 2. Usuários fazem apostas (COM NOVA LÓGICA DE PERFIL)
            for user in st.session_state.usuarios:
                ...
                if random.random() < prob_apostar_perfil:
                    ...
                    for _ in range(qtde_apostas_desejadas):
                        ...
                        jogo_escolhido = random.choice(jogos_nesta_rodada)
            ...
            # 3. Resolver Jogos
            for jogo in jogos_nesta_rodada:
            ...
            st.session_state.jogos_da_rodada_anterior = jogos_nesta_rodada
            
            # Salvar no histórico
            st.session_state.historico_jogos.append({
                "rodada": st.session_state.rodada_atual,
                "jogos": jogos_nesta_rodada.copy()
            })

            # 4. Liquidar Apostas (COM LÓGICA DE PERFIL)
            for aposta in apostas_nesta_rodada:
                jogo_correspondente = next(j for j in jogos_nesta_rodada if j["id_jogo"] == aposta["id_jogo"])
...
if not st.session_state.simulacao_iniciada:
    ...
    if len(df_historico) > 0:
        ...
        with g_col1:
            ...
            st.line_chart(df_historico_acum_plot[['Lucro na Rodada', 'Lucro Acumulado']], 
                          color=["#ff7f0e", "#1f77b4"]) # Laranja para rodada, Azul para acumulado
        ...
        with g_col3:
            ...
            if len(df_historico) > 1:  # Precisa de pelo menos 2 rodadas para mostrar evolução
                ...
                df_usuarios_evolucao['Lucrativos (%)'] = (df_usuarios_evolucao['usuarios_lucrativos'] / total_usuarios * 100).round(1)
                df_usuarios_evolucao['Zerados (%)'] = (df_usuarios_evolucao['usuarios_zerados'] / total_usuarios * 100).round(1)
                
                # Usar apenas as colunas de percentual
                df_usuarios_evolucao_perc = df_usuarios_evolucao[['Lucrativos (%)', 'Zerados (%)']].copy()
                st.line_chart(df_usuarios_evolucao_perc, color=["#1f77b4", "#d62728"])  # Azul, Vermelho
            else:
                st.write("Execute pelo menos 2 rodadas para ver a evolução.")
        ...
        with g_col4:
            ...
            if len(df_historico) > 1:
                ...
                for _, row in df_historico.iterrows():
                    ...
                    for perfil in LISTA_PERFIS:
                        linha_perfil[perfil] = row['usuarios_ativos_por_perfil'][perfil]
                    dados_perfil.append(linha_perfil)
                
                df_perfil_evolucao = pd.DataFrame(dados_perfil).set_index('rodada')
                cores_perfil = [PERFIS_CONFIG_DEFAULT[p]["cor"] for p in LISTA_PERFIS]
                st.line_chart(df_perfil_evolucao, color=cores_perfil)
            else:
                st.write("Execute pelo menos 2 rodadas para ver a evolução.")
    ...
    with st.expander("🔍 Explorar Jogos e Apostas por Rodada"):
    ...
    with st.expander("👤 Detalhes dos Usuários"):
    ...
    with st.expander("📊 Histórico Individual do Usuário"):
